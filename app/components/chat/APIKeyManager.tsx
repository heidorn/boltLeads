    ...
    <div className="flex items-center justify-between py-3 px-1">
      <div className="flex items-center gap-2 flex-1">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-bolt-elements-textSecondary">Chave da API {provider?.name}:</span>
          {!isEditing && (
            <div className="flex items-center gap-2">
              {apiKey ? (
                <>
                  <div className="i-ph:check-circle-fill text-green-500 w-4 h-4" />
                  <span className="text-xs text-green-500">Configurada aqui</span>
                </>
              ) : isEnvKeySet ? (
                <>
                  <div className="i-ph:check-circle-fill text-green-500 w-4 h-4" />
                  <span className="text-xs text-green-500">Configurada no .env.local</span>
                </>
              ) : (
                <>
                  <div className="i-ph:x-circle-fill text-red-500 w-4 h-4" />
                  <span className="text-xs text-red-500">Não configurada — defina aqui ou no .env.local</span>
                </>
              )}
            </div>
          )}
        ...
        {isEditing ? (
          ...
          <>
            ...
            {provider?.getApiKeyLink && !apiKey && (
              ...
              >
                <span className="text-xs whitespace-nowrap">{provider?.labelForGetApiKey || 'Obter chave'}</span>
