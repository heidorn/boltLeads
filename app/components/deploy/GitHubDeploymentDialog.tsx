...
export function GitHubDeploymentDialog({ isOpen, onClose, projectName, files }: GitHubDeploymentDialogProps) {
  ...
  const fetchRecentRepos = async (token: string) => {
    if (!token) {
      logStore.logError('Nenhum token do GitHub disponível');
      toast.error('Autenticação do GitHub necessária');
    ...
    try {
      ...
      while (hasMore) {
        ...
        if (!response.ok) {
          ...
          try {
            ...
            errorData = { message: 'Não foi possível interpretar a resposta de erro' };
          }

          if (response.status === 401) {
            toast.error('O token do GitHub expirou. Conecte sua conta de novo.');
            ...
            const resetDate = resetTime ? new Date(parseInt(resetTime) * 1000).toLocaleTimeString() : 'em breve';
            toast.error(`Limite de requisições da API do GitHub excedido. Redefinição: ${resetDate}`);
          } else {
            logStore.logError('Não foi possível carregar os repositórios do GitHub', {
              status: response.status,
              statusText: response.statusText,
              error: errorData,
            });
            toast.error(`Não foi possível carregar os repositórios: ${errorData.message || response.statusText}`);
          }
        ...
        try {
          ...
          logStore.logError('Não foi possível interpretar a resposta de repositórios do GitHub', { parseError });
          toast.error('Não foi possível interpretar os dados dos repositórios');
          setRecentRepos([]);
      ...
      logStore.logError('Não foi possível carregar os repositórios do GitHub', { error });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!sanitizedName || sanitizedName.length < 1) {
      toast.error('O nome do repositório precisa ter pelo menos um caractere alfanumérico');
      return;
    }

    if (sanitizedName.length > 100) {
      toast.error('O nome do repositório é longo demais (máximo de 100 caracteres)');
      return;
    }
    ...
    if (sanitizedName !== repoName) {
      ...
      toast.info(`Nome do repositório ajustado para: ${sanitizedName}`);
    }
    ...
    try {
      ...
      try {
        ...
        let confirmMessage = `O repositório "${repoName}" já existe. Deseja atualizá-lo? Isso vai adicionar ou modificar arquivos no repositório.`;

        // Add visibility change warning if needed
        if (existingRepo.private !== isPrivate) {
          const visibilityChange = isPrivate
            ? 'Isso também vai mudar o repositório de público para privado.'
            : 'Isso também vai mudar o repositório de privado para público.';
      ...
      try {
          ...
          try {
            ...
            const errorMsg =
              typeof createRefError === 'object' && createRefError !== null && 'message' in createRefError
                ...
                : 'Erro desconhecido';
            throw new Error(`Não foi possível criar a referência do Git: ${errorMsg}`);
          }
        }
      } catch (gitError) {
        ...
        const gitErrorMsg =
          typeof gitError === 'object' && gitError !== null && 'message' in gitError
            ...
            : 'Erro desconhecido';
        throw new Error(`Não foi possível concluir as operações do Git: ${gitErrorMsg}`);
      }
      ...
      let errorMessage = 'Não foi possível enviar para o GitHub';
      ...
      if (error instanceof Error) {
        ...
        if (errorMsg.includes('network') || errorMsg.includes('fetch failed') || errorMsg.includes('connection')) {
          ...
          errorMessage =
            'Não foi possível autenticar no GitHub. Verifique seu token de acesso em Configurações > Conexões.';
        } else if (errorMsg.includes('403') || errorMsg.includes('forbidden')) {
          errorMessage =
            'Acesso negado. Seu token do GitHub pode não ter permissão para criar ou modificar repositórios.';
        } else if (errorMsg.includes('404') || errorMsg.includes('not found')) {
          errorMessage = 'Repositório ou recurso não encontrado. Verifique o nome do repositório e suas permissões.';
        } else if (errorMsg.includes('422') || errorMsg.includes('validation failed')) {
          if (errorMsg.includes('name already exists')) {
            errorMessage = 'Já existe um repositório com esse nome na sua conta. Escolha outro nome.';
          } else {
            errorMessage =
              'Não foi possível validar o repositório. Verifique o nome e as configurações do repositório.';
          }
        } else if (errorMsg.includes('rate limit') || errorMsg.includes('429')) {
          errorMessage = 'Limite de requisições da API do GitHub excedido. Aguarde um momento e tente de novo.';
          isRetryable = true;
        } else if (errorMsg.includes('timeout')) {
          errorMessage = 'A requisição expirou. Verifique sua conexão e tente de novo.';
          isRetryable = true;
        } else {
          errorMessage = `Erro do GitHub: ${error.message}`;
        }
      } else if (typeof error === 'object' && error !== null) {
        // Octokit errors
        if ('message' in error) {
          errorMessage = `Erro da API do GitHub: ${error.message as string}`;
        }
      ...
      const finalMessage = isRetryable ? `${errorMessage} Clique para tentar de novo.` : errorMessage;
        ...
        <Dialog.Portal>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-[9999]">
            ...
            >
              ...
              >
                <Dialog.Title className="sr-only">Código enviado para o GitHub</Dialog.Title>
        ...
        <Dialog.Portal>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-[9999]">
            ...
            >
              ...
              >
                <Dialog.Title className="sr-only">Conexão com o GitHub necessária</Dialog.Title>
                <div className="relative text-center space-y-4">
                  ...
                  <h3 className="text-lg font-medium text-bolt-elements-textPrimary dark:text-bolt-elements-textPrimary-dark">
                    Conexão com o GitHub necessária
                  </h3>
                  <p
                  ...
                  >
                    Para fazer deploy do seu código no GitHub, conecte antes sua conta do GitHub.
                  </p>
                ...
                <div className="flex items-center gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p
                      ...
                      className="text-sm text-bolt-elements-textSecondary dark:text-bolt-elements-textSecondary-dark"
                    >
                      Envie seu código para um repositório do GitHub, novo ou existente
                    </p>
                  </div>
                  <Dialog.Close asChild>
                    <button
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        title="O nome do repositório pode ter letras, números, hifens, sublinhados e espaços"
                      />
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {recentRepos.length === 0 && !isFetchingRepos ? (
                      <EmptyState
                        ...
                        description="Não há repositórios na sua conta do GitHub."
                        variant="compact"
                      />
                    ) : (
